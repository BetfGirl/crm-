	...
	<script>
		...
		layui.use('table', function() {
			...
			table.render({
				elem : '#demo',
				height : 462,
				url : '../Operator/index.action',//数据接口
				toolbar : '#toolbarDemo',
				page : true //开启分页
				,
				cols : [ [ //表头
				{
					...
					field : 'workgroup',//dao层和表关联  使用别名
					title : '工作组',
					width : 120
				}, {
					field : 'powername',//调用model里面的 getpowername方法返回
					title : '权限',
					width : 120
				}, {
					field : 'statuss',//调用model里面的 getstatuss方法返回
					title : '状态',
					width : 120
				}, {
					fixed : 'right',
			...
			table.on('tool(test)', function(obj) {
				...
				if (obj.event === 'del') { ///lay-event 属性
					...
					myconfirm("刪除？",function(){
						$.post("../Operator/delete.action", {cmd : "delete",id : data.id}, 
								function(json) {
							reload('demo');
					...
					openFrames('../Operator/edit.jsp?id='+data.id);
				}
			});
