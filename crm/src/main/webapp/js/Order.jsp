...
<a class="layui-btn layui-btn-xs" lay-event="checkdeatils">查看详细</a>
</script>
		...
		layui.use('table', function() {
			...
			table.render({
				...
				/* {
					...
					field : 'compdate',//dao层和表关联  使用别名
					title : '业绩日期',
					width : 106
				}, {
					field : 'orderdate',//调用model里面的 getpowername方法返回
					title : '合同时间',
					width : 88
				}, {
					field : 'startdate',
			...
			table.on('tool(test)', function(obj) {
				...
				if (obj.event === 'del') { ///lay-event 属性
					...
					myconfirm("刪除？",function(){
						$.post("../Order/delete.action", {cmd : "delete",id : data.id}, 
								function(json) {
							reload('demo');
					...
					openFrame('../Order/edit.jsp?id='+data.id);
				}
			});
			...
			table.on('toolbar(test)', function(obj) {
				if (obj.event === 'search') {
					...
					reload('demo',{txt : txt});
				} else if(obj.event === 'checkdeatils'){
					openFrame("../Order/checkdeatils.jsp");
				} else {
					openFrame("../Order/edit.jsp");
				}
			});
